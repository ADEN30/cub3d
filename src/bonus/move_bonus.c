...
static void	move_a(t_vars *vars)
{
	...
	xy[0] = vars->pers->x + cos(vars->pers->angle + (90 * M_PI / 180)) * SPEED;
	xy[1] = vars->pers->y - sin(vars->pers->angle + (90 * M_PI / 180)) * SPEED;
...
static void	move_w(t_vars *vars)
{
	...
	if (vars->pers->angle < 0)
		...
		vars->pers->angle = vars->pers->angle - 2 * M_PI;
	xy[0] = vars->pers->x + vars->pers->deltax * SPEED;
	xy[1] = vars->pers->y - vars->pers->deltay * SPEED;
	if (check_collision(vars, xy[0], xy[1]))
		return ;
	if (vars->pers->points)
		free(vars->pers->points);
