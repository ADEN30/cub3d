...
/*   Updated: 2024/01/12 15:04:28 by jmathieu         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	looking_left(t_vars *vars, double *r, double *ofs, char *face)
{
	...
	r[0] = (((int)vars->pers->x / vars->dim) * vars->dim) - 0.0001;
	ofs[0] = -vars->dim;
...
static void	looking_right(t_vars *vars, double *r, double *ofs, char *face)
{
	...
	tg = tan(vars->pers->angle);
	r[0] = (((int)vars->pers->x / vars->dim) * vars->dim) + vars->dim;
	ofs[0] = vars->dim;
	if (vars->pers->angle == M_PI / 2)
	{
		r[1] = vars->pers->y;
...
int	vertical_intersection(t_vars *vars, double *v, char *face)
{
	...
	if (find_wall_vh(vars, v, ray, ofs))
		return (1);
	return (0);
}
